...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($db,"select * from equipment where p_id='$id'");
...
if(isset($_POST['submit'])){
	...
	$res=mysqli_query($db,"select * from equipment where p_name='$p_name'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['p_image']['name']!=''){
				...
				move_uploaded_file($_FILES['p_image']['tmp_name'],EQUIPMENT_IMAGE_SERVER_PATH.$p_image);
				$update_sql="update equipment set p_image='$p_image',p_name='$p_name',	p_weight='$	p_weight',p_price='$p_price',titleone='$titleone',desone='$desone',titletwo='$titletwo',destwo='$destwo',titlethree='$titlethree',desthree='$desthree' where p_id='$p_id'";
			}else{
				$update_sql="update equipment set p_image='$p_image',p_name='$p_name',	p_weight='$p_weight',p_price='$p_price',titleone='$titleone',desone='$desone',titletwo='$titletwo',destwo='$destwo',titlethree='$titlethree',desthree='$desthree' where p_id='$p_id'";
			}
			...
			$p_image=rand(111111111,999999999).'_'.$_FILES['p_image']['name'];
			move_uploaded_file($_FILES['p_image']['tmp_name'],EQUIPMENT_IMAGE_SERVER_PATH.$p_image);
			mysqli_query($db,"insert into equipment(p_image,p_name,p_weight,p_price,titleone,desone,titletwo,destwo,titlethree,desthree,	p_cat) values('$p_image','$p_name','$p_weight','$p_price','$titleone','$desone','$titletwo','$destwo','$titlethree','$desthree',4)");
		}
		header('location:equipments.php');
		die();
	}
