...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($db,"select * from seed where p_id='$id'");
...
if(isset($_POST['submit'])){
	...
	$offer=get_safe_value($db,$_POST['offer']);
	...
	$res=mysqli_query($db,"select * from seed where p_name='$p_name'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['p_image']['name']!=''){
				...
				move_uploaded_file($_FILES['p_image']['tmp_name'],SEED_IMAGE_SERVER_PATH.$p_image);
				$update_sql="update seed set p_image='$p_image',p_name='$p_name',	p_weight='$p_weight',p_price='$p_price',offer='$offer',titleone='$titleone',desone='$desone',titletwo='$titletwo',destwo='$destwo',titlethree='$titlethree',desthree='$desthree' where p_id='$p_id'";
			}else{
				$update_sql="update seed set p_image='$p_image',p_name='$p_name',	p_weight='$p_weight',p_price='$p_price',offer='$offer',titleone='$titleone',desone='$desone',titletwo='$titletwo',destwo='$destwo',titlethree='$titlethree',desthree='$desthree' where p_id='$p_id'";
			}
			...
			$p_image=rand(111111111,999999999).'_'.$_FILES['p_image']['name'];
			move_uploaded_file($_FILES['p_image']['tmp_name'],SEED_IMAGE_SERVER_PATH.$p_image);
			mysqli_query($db,"insert into seed(p_image,p_name,p_weight,p_price,offer,titleone,desone,titletwo,destwo,titlethree,desthree,	p_cat) values('$p_image','$p_name','$p_weight','$p_price','$offer','$titleone','$desone','$titletwo','$destwo','$titlethree','$desthree',2)");
		}
		header('location:seed.php');
		die();
	}
                        ...
                        <form method="post" enctype="multipart/form-data">
							<div class="card-body card-block">
							   <div class="form-group">
								...
								<div class="form-group">
									<label for="categories" class=" form-control-label">Offer</label>
									<input type="text" name="offer" placeholder="Enter product offer" class="form-control" required value="<?php echo $offer?>">
								</div>
